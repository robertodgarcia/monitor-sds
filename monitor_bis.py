...
# (Opcional, mas recomendado) evita travar indefinidamente em ambiente de CI
...
def decodificar_texto(header_text):
    ...
    for content, encoding in decoded_list:
        if isinstance(content, bytes):
            try:
                texto_final += content.decode(encoding or 'utf-8', errors='ignore')
            except Exception:
...
def verificar_emails():
    ...
    # 1. Checagem de Variáveis Globais
    if not EMAIL_USER or not EMAIL_PASS:
        print("ERRO: Secrets não configurados. EMAIL_USER ou EMAIL_PASS está vazio.")
        return

    try:
        # TLS (modo compatibilidade): útil quando o OpenSSL do runner é mais rígido
        ssl_context = ssl.create_default_context()

        # 1) Permite conjuntos de cifra mais antigos (reduz exigência de segurança).
        #    OBS: sintaxe mais compatível é "DEFAULT:@SECLEVEL=1"
        try:
            ssl_context.set_ciphers("DEFAULT:@SECLEVEL=1")
        except Exception as _e:
            print(" [TLS] Aviso: não foi possível setar ciphers/SECLEVEL=1:", _e)

        # 2) Permite conexão com servidores com handshake/renegociação legacy (OpenSSL 3).
        if hasattr(ssl, "OP_LEGACY_SERVER_CONNECT"):
            try:
                ssl_context.options |= ssl.OP_LEGACY_SERVER_CONNECT
            except Exception as _e:
                print(" [TLS] Aviso: não foi possível habilitar OP_LEGACY_SERVER_CONNECT:", _e)
        ...
        for e_id in email_ids:
            ...
            assunto_upper = (assunto or "").upper().strip()
            ...
            conteudo_analisado = ""
            anexos_encontrados = []  # (nome, payload, conteudo_anexo)
            ...
            if msg.is_multipart():
                for part in msg.walk():
                    ...
                    if filename:
                        ...
                        if "pdf" in ctype:
                            ...
                            filename = "corpo_email.html"
                        elif "text/plain" in ctype:
                            payload = part.get_payload(decode=True)
                            if payload:
                                try:
                                    conteudo_analisado += payload.decode('utf-8', errors='ignore')
                                except Exception:
                                    pass
                            continue
                        else:
                ...
                if "html" in ctype:
                    conteudo_anexo = extrair_texto_html(payload or b"")
                    anexos_encontrados.append(("email_completo.html", payload or b"", conteudo_anexo))
                else:
                    try:
                        conteudo_anexo = (payload or b"").decode('utf-8', errors='ignore')
                    except Exception:
                        conteudo_anexo = ""
            ...
            # --- LÓGICA DE ALERTA ---
            conteudo_upper = (conteudo_analisado or "").upper()
...
if __name__ == "__main__":
    print("🤖 Monitor v7.2 (GitHub Actions + TLS compat)...")
    verificar_emails()
